...
#[derive(Parser, Debug)]
#[command(name = "kubuno-code", version, about = "Module IDE Kubuno")]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    match settings.logging.format {
        kubuno_code::config::LogFormat::Json   => subscriber.json().init(),
        kubuno_code::config::LogFormat::Pretty => subscriber.init(),
    }

    tracing::info!("Kubuno Code v{} démarrage…", env!("CARGO_PKG_VERSION"));

    // Sécurité : interdire définitivement l'exécution de tout processus externe.
    // Le module ne lance aucun binaire dans son fonctionnement normal ; ce verrou
    // garantit qu'aucun utilisateur (ni régression future) ne peut faire exécuter
    // une commande sur la machine hôte. À poser avant toute autre initialisation.
    kubuno_seccomp::lock_down_process_execution("code");

    // Créer les répertoires de données
    for dir in &[&settings.code.projects_root, &settings.code.extensions_dir] {
    ...
    // Heartbeat toutes les 30s
    {
        ...
        tokio::spawn(async move {
            loop {
                ...
                let url    = format!("{}/internal/modules/code/heartbeat", settings2.core.url);
    ...
    tracing::info!("Kubuno Code démarré sur http://{addr}");
...
async fn register_with_core(http: &Client, settings: &Settings) {
    ...
    let display_name  = manifest.as_ref().map(|m| m.module.display_name.as_str()).unwrap_or("Code").to_string();
    ...
    let sidebar_items: Vec<Value> = manifest.as_ref()
        ...
        .unwrap_or_else(|| vec![
            json!({ "id": "code", "label": "Code", "icon": "Code2", "path": "/code", "position": 60 }),
        ]);
    ...
    for attempt in 1u32.. {
            ...
            .await
        {
            Ok(resp) if resp.status().is_success() => {
                tracing::info!("Module code enregistré auprès du core");
                return;
            }
