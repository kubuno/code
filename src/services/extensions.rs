...
pub async fn install(
    ...
    // Key generated by the process (see the note in the projects handler): the
    // row is then read back by that id to return `is_enabled` and `installed_at`
    // as the database defaulted them.
    let id = new_id();
    state
        .db
        .execute(
    ...
    let ext = state
        .db
        .fetch_one_as::<Extension>("SELECT * FROM code.extensions WHERE id = $1", params![id])
        .await?;
