...
<p align="center">
  <img src=".github/logo.png" alt="Kubuno Code logo" width="128" height="128">
</p>
...
**Kubuno Code — IDE web collaborative**

A module for [Kubuno](https://github.com/kubuno/core), the self-hosted, libre (AGPLv3) cloud platform.

## Architecture

A standalone Rust process that registers with the [core](https://github.com/kubuno/core) at startup; the core proxies its routes (`/api/v1/code/*`) and serves its runtime-loaded React frontend bundle.

- **Backend** — `src/`: Axum + SQLx (PostgreSQL, schema `code`); migrations in `migrations/`.
...
**Native packages** for every supported platform are attached to each [GitHub Release](https://github.com/kubuno/code/releases): Debian/Ubuntu (`.deb`), Fedora/RHEL/openSUSE (`.rpm`), Windows (NSIS installer), and macOS (`.pkg`). They all install the module into an existing Kubuno core installation and restart the service.
...
bash build_deb.sh                          # → dist/kubuno-code_*.deb
```
...
```bash
bash build_rpm.sh                          # → dist/kubuno-code-*.rpm   (Fedora/RHEL/openSUSE)
bash build_windows.sh                      # → dist/kubuno-code-setup-*-x64.exe (NSIS; cargo-xwin from Linux)
bash build_macos.sh                        # → dist/kubuno-code-*-arm64.pkg     (run on a Mac)
```
