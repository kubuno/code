...
- 🗂️ **Projects & explorer** — organise your work into projects with a file-tree explorer; create, rename, move and delete files and folders, with a per-user project cap an administrator can set.
...
- ⚙️ **Instance controls** — a maximum file size for opening and saving, the extension-registry endpoint, the per-user project ceiling, and a strict remote-clone policy (allowed-hosts list) editable from the core admin console.
...
- **Backend** — `src/`: Axum + SQLx (PostgreSQL, dedicated schema `code`); migrations in `migrations/`. Files, projects, Git and the extension registry are handled server-side; nothing is ever executed.
...
```bash
sudo kubuno modules:install kubuno-code-<version>-<os>-<arch>.kbpkg
...
Rust 2021 · Axum · Tokio · SQLx (PostgreSQL 16) — React 19 · TypeScript · Vite · Tailwind CSS v4 · Zustand · React Query · Monaco editor.
